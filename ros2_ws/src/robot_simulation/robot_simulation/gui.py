...
try:
    ...
    # --- THE FIXED ROS 2 ENVIRONMENT RESOLUTION ---
    # Dynamically resolve the workspace setup.bash instead of guessing paths via raw bash strings
    pkg_prefix = get_package_prefix('robot_simulation')
    workspace_install_dir = os.path.dirname(pkg_prefix)
    local_setup_script = os.path.join(workspace_install_dir, 'setup.bash')
    
    ROS2_SETUP_CMD = f"source /opt/ros/humble/setup.bash && source {local_setup_script}"
except Exception as e:
    print(f"CRITICAL: Failed to locate package share/prefix directory. Error: {e}")
    IMAGE_DIR = ""
    ROS2_SETUP_CMD = "source /opt/ros/humble/setup.bash"
