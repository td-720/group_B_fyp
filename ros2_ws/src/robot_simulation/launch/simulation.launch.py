...
def launch_setup(context, *args, **kwargs):
    ...
    # --- THE FIX: Capture the PID parameters from the GUI ---
    ...
    # Bag logging includes EKF output and IMU
    bag_name = 'obstacle_run_' + datetime.datetime.now().strftime("%Y_%m_%d-%H_%M_%S")
    rosbag_record = ExecuteProcess(
        cmd=['ros2', 'bag', 'record', '-o', bag_name, '--use-sim-time',
             '/scan', '/diffdrive_controller/cmd_vel_unstamped', 
            ...
            if scenario_name_str == 'teleop_node':
                ...
                scenario_brain = Node(
                    ...
                    parameters=[{
                        'use_sim_time': True,
                        'relative_waypoints': LaunchConfiguration('relative_waypoints'),
                        # --- THE FIX: Inject the PID parameters directly into the Node ---
                        'kp': kp_arg,
                        'ki': ki_arg,
                        'kd': kd_arg
                        # -----------------------------------------------------------------
...
def generate_launch_description():
    return LaunchDescription([
        ...
        # --- THE FIX: Declare the arguments so the bouncer lets them in ---
        DeclareLaunchArgument('kp', default_value='1.5'),
        DeclareLaunchArgument('ki', default_value='0.02'),
        DeclareLaunchArgument('kd', default_value='3.0'),
        # ------------------------------------------------------------------
