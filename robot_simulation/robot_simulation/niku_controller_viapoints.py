...
class NikuController(Node):
    def __init__(self):
        ...
        self.csv_filename = os.path.join(log_dir, f'run_{timestamp}_tracking.csv')
        self.csv_file = open(self.csv_filename, mode='w', newline='', buffering=1) 
        self.csv_writer = csv.writer(self.csv_file)
    ...
    def generate_path(self):
        ...
        local_pts = [[0.0, 0.1]] + self.relative_via_points
        local_path_x = []
        local_path_y = []
    ...
    # --- MODIFIED: Return ref_x and ref_y for logging ---
    def _interpolate_lookahead(self, Ld: float):
        path_x = list(self.path_x)
        path_y = list(self.path_y)
